{{-- July 2026 UI cleanup: Student listed first in the role dropdown
     (sorted here in the view, controller query untouched), password hint
     shown up front to match the min:8 rule, autocomplete attributes.
...
<head>
    ...
    <title>Register - CSPC Report Concern</title>
    ...
    <style>
        ...
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .card{background:#fff;border:1px solid var(--line);border-radius:18px;
            box-shadow:0 24px 60px -18px rgba(13,27,62,.28);padding:2.4rem;width:100%;max-width:440px}
        ...
        .back{display:block;text-align:center;color:var(--brand);font-size:.85rem;font-weight:600;
...
<body>
    <div class="wrap">
        <aside class="brand">
            ...
            <div class="hero">
                <h1>Request<br>an account.</h1>
                <p>Registration is open to CSPC students, faculty, staff, and administrators. Students get instant access; other roles are reviewed by an admin before access is granted.</p>
            </div>
        ...
        <main class="formside">
            <div class="card">
                ...
                <p class="sub">Students get instant access. Other roles are reviewed by an admin before sign-in.</p>
                ...
                <form action="{{ route('register.post') }}" method="POST">
                    ...
                    <div class="fg">
                        ...
                        <input type="text" id="name" name="name" value="{{ old('name') }}"
                               placeholder="Juan Dela Cruz" autocomplete="name" required autofocus>
                    </div>
                    <div class="fg">
                        <label for="email">Email address</label>
                        <input type="email" id="email" name="email" value="{{ old('email') }}"
                               placeholder="you@cspc.edu" autocomplete="email" required>
                        <div class="hint">Must be a CSPC email (@cspc.edu or @my.cspc.edu.ph).</div>
                    </div>
                    <div class="fg">
                        <label for="role_id">I am a...</label>
                        <select id="role_id" name="role_id" required>
                            <option value="">Select a role</option>
                            {{-- Students are the overwhelming majority of sign-ups,
                                 so their option comes first; the rest stay alphabetical. --}}
                            @foreach ($roles->sortBy(fn ($r) => $r->name === 'Student' ? 0 : 1)->values() as $role)
                    ...
                    <div class="fg">
                        <label for="department">Department / office <span style="font-weight:400;color:var(--muted)">(optional)</span></label>
