...
class AggressiveQaTest extends TestCase
{
    ...
    public function test_student_cannot_open_another_students_concern(): void
    {
        ...
        $r=$this->actingAs($this->u('student@cspc.edu'))->get("/concerns/{$other->id}");
    ...
    {
        $c=$this->mk(['status'=>'in_progress','assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('student@cspc.edu'))->delete("/concerns/{$c->id}");
    ...
    {
        $c=$this->mk(['is_anonymous'=>true,'assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->get("/concerns/{$c->id}");
    ...
    public function test_admin_cannot_read_untouched_mental_health(): void
    {
        ...
        $r=$this->actingAs($this->u('admin@cspc.edu'))->get("/concerns/{$mh->id}");
    ...
    {
        $mh=$this->mk(['category'=>'Mental Health / Personal']);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->get("/concerns/{$mh->id}");
    ...
    {
        $c=$this->mk(['status'=>'resolved','assigned_to'=>$this->u('staff@cspc.edu')->id,'urgency'=>'Low']);
        $this->actingAs($this->u('staff@cspc.edu'))->patch("/concerns/{$c->id}",['status'=>'in_progress','urgency'=>'High']);
        $c->refresh();
    ...
    {
        $c=$this->mk(['assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->patch("/concerns/{$c->id}",['status'=>'referred','referred_to'=>'','urgency'=>'Low']);
    ...
    {
        $c=$this->mk(['assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->patch("/concerns/{$c->id}",['status'=>'HACKED','urgency'=>'Low']);
    ...
    {
        $c=$this->mk(['assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->patch("/concerns/{$c->id}",['status'=>'in_progress','urgency'=>'SUPERCRITICAL']);
    ...
    {
        $r=$this->actingAs($this->u('student@cspc.edu'))->post("/concerns",[
    ...
    {
        $r=$this->actingAs($this->u('student@cspc.edu'))->post("/concerns",[
            'category'=>'Academic','department'=>'Hogwarts','description'=>'x',
        ]);
        $r->assertSessionHasErrors('department');
        $this->line("[validation] invalid department rejected: yes");
    }
    ...
    {
        for($i=0;$i<6;$i++){
            $r=$this->post('/login',['email'=>'admin@cspc.edu','password'=>'wrongpass']);
        }
        $this->line("[security] 6th bad login status: ".$r->getStatusCode()." (429 = throttled)");
        $this->assertEquals(429,$r->getStatusCode(),'Login should throttle after repeated failures');
    }
    ...
    public function test_description_xss_is_escaped_on_render(): void
    {
        ...
        $c=$this->mk(['description'=>$payload,'assigned_to'=>$this->u('staff@cspc.edu')->id]);
        $r=$this->actingAs($this->u('staff@cspc.edu'))->get("/concerns/{$c->id}");
    ...
    public function test_student_cannot_mass_assign_user_id_on_create(): void
    {
        ...
        $victim=$this->u('student2@cspc.edu');
        ...
        $this->line("[mass-assign] concern owner id=".($c?$c->user_id:'n/a')." (want ".$this->u('student@cspc.edu')->id.", not {$victim->id})");
        $this->assertNotNull($c);
        $this->assertEquals($this->u('student@cspc.edu')->id,$c->user_id,'Owner must be the authenticated user, not spoofed');
    }
}