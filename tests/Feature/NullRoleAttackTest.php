...
class NullRoleAttackTest extends TestCase {
    ...
    public function test_null_role_user_does_not_500_anywhere(): void {
        $nr=User::create(['name'=>'NR','email'=>'nr@my.cspc.edu.ph','password'=>bcrypt('x'),'role_id'=>null]);
        $c=Concern::create(['user_id'=>User::where('email','student@my.cspc.edu.ph')->first()->id,'category'=>'Academic','department'=>'College of Computer Studies','description'=>'x','urgency'=>null,'status'=>'submitted','is_anonymous'=>false]);
        foreach(['/concerns',"/concerns/{$c->id}",'/dashboard'] as $url){
