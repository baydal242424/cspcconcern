...
class QaAuditTest extends TestCase
{
    ...
    {
        $this->actingAs($this->u('student@cspc.edu'))->post('/concerns', array_merge([
    ...
    public function test_counselor_sees_assigned_concern(): void
    {
        ...
        $resp = $this->actingAs($this->u('counselor@cspc.edu'))->get('/concerns');
        $resp->assertOk();
    ...
    public function test_staff_sees_assigned_concern(): void
    {
        ...
        $this->actingAs($this->u('staff@cspc.edu'))->get('/concerns')
    ...
    {
        $c = $this->submit();
        $this->actingAs($this->u('student2@cspc.edu'))->get("/concerns/{$c->id}")->assertForbidden();
    }
    ...
    {
        $c = $this->submit();
        $this->actingAs($this->u('student@cspc.edu'))->get("/concerns/{$c->id}")
